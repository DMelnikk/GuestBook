...
function login(array $data): bool
{
    ...
    $_SESSION['success'] = 'You have successfully loggined';
    return true;
...
function save_message(array $data): bool {
    global $db;
    if (!check_auth()) {
        $_SESSION['errors'] = 'Requiered Authefication';
        return false;
    }
...
function get_messages(int $start , int $per_page): array
{
    ...
    $stmt = $db->prepare("SELECT messages.*,
       DATE_FORMAT(messages.created_at,'%d.%m.%Y %H:%i') AS created_at2 , users.name 
            FROM messages 
           JOIN users 
           ON users.id = messages.user_id {$where} ORDER BY id DESC LIMIT $start,$per_page");
    $stmt->execute();
